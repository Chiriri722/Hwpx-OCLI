...
> P2(G5 스타일 매핑)는 후속 공개 281개 코퍼스와 한컴 2020 네이티브 DOCX
> 오라클을 확보해 T2-5/ADR-0009에서 완료했다. G6 문자 치환은 신뢰할 대응표가
> 생길 때만 재개하는 evidence-gated 항목이다.
...
#### G5. `styleIDRef`/`hh:heading` → docx `style` — **T2-5 완료**
...
**당시 결론**: 우선순위 기준 2번("실측 표본에서 몇 건 발생하는가")에 따라 보류한다.
개요 스타일을 실제로 쓰는 문서(보고서·논문 계열)를 표본에 확보하면 그때
착수한다. 그때는 **스타일 정의 생성까지** 범위에 넣어야 한다.

당시 제한된 표본에서는 `style`을 내보내지 않는 것이 옳은 동작임을 확인한 것도 결과다.

**2026-08-29 재개·완료**: 후속 공개 코퍼스에서 활성 이름/개요 스타일을 확보했고,
한컴 2020의 네이티브 DOCX 변환으로 직접 NONE과 스타일 OUTLINE, 숫자 ID,
`lockForm`, 암묵 `outlineShapeIDRef=0` 동작을 실측했다. 커밋 `50adcc31`은 실제
사용 스타일과 다음 스타일 의존성의 정의를 먼저 만들고 문단 참조를 적용하며,
직접 서식을 별도 override로 유지한다. 공개 281개 코퍼스의 기존 기준선과 실제
...
| 항목 | 이전 순위 | 새 순위 | 근거 |
|---|---|---|---|
| 각주/미주 | 3 | T2-1/T2-3 완료 | 후속 한컴 생성 표본과 공식 구조로 본문·구역 정책 경계를 확정했다 |
| 목록 번호 매기기 | 4 | T2-4 완료 | 후속 공개 `ParaHead.hwpx`와 한컴 DOCX 오라클로 동적 번호 구조를 검증했다 |
| 머리말/꼬리말 | 5 | T2-3 완료 | 후속 공개·한컴 표본으로 구역 story와 페이지 필드를 검증했다 |
...
| 도형·차트 | - | 보류 | 실제 표본과 호스트 투영 경계가 아직 필요하다 |

이 표의 최초 판단은 5개 표본만을 근거로 했고, 이후 확보한 공개·한컴 생성 표본에 따라
T2-1~T2-5를 재개했다. 현재 우선순위와 완료 상태의 정본은
`specs/001-hancom-unified/task-plan.md`다. 여전히 표본이 없는 도형·차트는 추정으로
지원하지 않는다.
...
| R3 | `hp:switch`/`hp:case`/`hp:default` 구조를 우연히 잘 처리하고 있다 | 서로 다른 case/default 값을 둔 회귀 테스트로 마지막 `hp:default` 선택을 고정했다 |
| R4 | 누름틀 `Command` 문자열 형식 근거가 2개 문서뿐 | 파싱 실패 시 안내문만 생략하고 필드는 만든다. 원문을 버리지 않는다 |
| R5 | 코퍼스 문서의 저장소 포함 여부 | P-1 참고. 익명화 또는 외부 경로 |
| R6 | Windows/Linux 실행 | run `31572303544`, `31700156231`의 기존 HWPX·H3 게이트는 성공. H1 run `31890284597`, `32793306250`는 OfficeCLI 1.0.143 오류 처리에 가려 실패했다. 1.0.145 고정 자산은 로컬 양 OS smoke를 통과했으며 workflow 재실행이 필요 |
...
- **바이너리 HWP 파서를 자체 구현하거나 벤더링하는 일.** 후속 결정에서 선택적
  ...
  로컬에 구현했고 양 OS 로컬 `.hwp` discovery·`view`도 통과했다. 새
  Linux/Windows 원격 결과는 `04-hwp-support-plan.md`에서 계속 추적한다
  (정정된 ADR-2, ADR-5).
