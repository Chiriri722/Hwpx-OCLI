...
> P2(G5 스타일 매핑)와 G6 문자 치환은 대표 표본·신뢰할 대응표가 생길 때만
> 재개하는 evidence-gated 항목이며, 현재 구현 대상으로 남아 있지 않다.
...
T2-1~T2-4를 재개했다. 현재 우선순위와 완료 상태의 정본은
...
후속     G5 스타일 매핑            (P2, 대표 코퍼스 확보 때만 재개)
       프로세스 정비 (5절)
```
