...
//! RHWP `src/renderer/equation/tokenizer.rs`를 커밋
//! `496333b27d21ddb9114ba9ae340bcb895870c9a7`에서 가져와 인접 명령과 위치
//! 추적을 보완했다. 라이선스는 `plugins/hancom/NOTICE`에 있다.
...
const GLUE_SAFE: &[&str] = &[
    // 공식 r1.3 기본 함수 중 접두 분리가 명확한 이름. 함수 인자를 붙여 쓰는
    // `logx+sinx`가 공식 예시다. arg/max/min/lim 및 if/for/and 같은 짧은
    // 예약어는 `argmax`, `limit`, 일반 식별자와 모호하므로 독립 토큰만 인식한다.
    "arcsinh",
    "arcsin",
    "arccos",
    "arctan",
