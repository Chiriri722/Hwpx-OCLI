...
//! RHWP `src/renderer/equation/parser.rs`를 커밋
//! `496333b27d21ddb9114ba9ae340bcb895870c9a7`에서 가져와 완전 소비 검사와
//! 손실 없는 구조 파싱을 보완했다. 라이선스는 `plugins/hancom/NOTICE`에 있다.

use super::ast::*;
use super::symbols::{
    is_big_operator, is_function, is_structure_command, lookup_function, lookup_symbol,
...
impl EqParser {
    ...
    /// 엄격 호출자가 최상위 입력을 전부 소비했는지 확인할 수 있는 진입점.
    pub fn parse_checked(&mut self) -> Result<EqNode, usize> {
        let node = self.parse_expression();
        if self.at_end() {
            Ok(node)
        } else {
            Err(self.current().map_or(self.pos, |token| token.pos))
        }
    }

    /// OVER/ATOP/SUP/SUB/LSUB/LSUP/CHOOSE 중위 연산자 처리. 현재 토큰이 해당 명령이면
    /// children의 마지막 요소를 pop하여 다음 피연산자와 결합한다.
    ...
    fn try_consume_infix_over_atop(&mut self, children: &mut Vec<EqNode>) -> bool {
        ...
        let is_sup = Self::cmd_eq(val, "SUP");
        let is_sub = Self::cmd_eq(val, "SUB");
        let is_lsup = Self::cmd_eq(val, "LSUP");
        let is_lsub = Self::cmd_eq(val, "LSUB");
        let is_choose = Self::cmd_eq(val, "CHOOSE");
        if !is_over && !is_atop && !is_sup && !is_sub && !is_lsup && !is_lsub && !is_choose {
            return false;
        }
        self.pos += 1;
        let left = children.pop().unwrap_or(EqNode::Empty);
        ...
        children.push(if is_atop {
            ...
            match left {
                EqNode::Subscript { base, sub } => EqNode::SubSup {
                    base,
                    sub,
                    sup: Box::new(right),
                },
                base => EqNode::Superscript {
                    base: Box::new(base),
                    sup: Box::new(right),
                },
            }
            ...
            match left {
                EqNode::Superscript { base, sup } => EqNode::SubSup {
                    base,
                    sub: Box::new(right),
                    sup,
                },
                base => EqNode::Subscript {
                    base: Box::new(base),
                    sub: Box::new(right),
                },
            }
            ...
            match left {
                EqNode::LeftSubscript { base, sub } => EqNode::LeftSubSup {
                    base,
                    sub,
                    sup: Box::new(right),
                },
                base => EqNode::LeftSuperscript {
                    base: Box::new(base),
                    sup: Box::new(right),
                },
            }
            ...
            match left {
                EqNode::LeftSuperscript { base, sup } => EqNode::LeftSubSup {
                    base,
                    sub: Box::new(right),
                    sup,
                },
                base => EqNode::LeftSubscript {
                    base: Box::new(base),
                    sub: Box::new(right),
                },
            }
    ...
    fn parse_command(&mut self, cmd: &str) -> EqNode {
        ...
        if matches!(
            ...
            let symbol = lookup_symbol(cu)
                ...
                .unwrap_or("∫")
                .to_string();
            return self.parse_integral(symbol);
        }
    ...
    /// 공식 HWP 수식의 적분 구간 표기(`INT FROM a TO b`)를 일반 첨자로 낮춘다.
    fn parse_integral(&mut self, symbol: String) -> EqNode {
        let base = EqNode::MathSymbol(symbol);
        let mut sub = None;
        let mut sup = None;

        if self.current_type() == TokenType::Command && Self::cmd_eq(self.current_value(), "FROM") {
            self.pos += 1;
            sub = Some(Box::new(self.parse_single_or_group()));
        }
        if self.current_type() == TokenType::Command && Self::cmd_eq(self.current_value(), "TO") {
            self.pos += 1;
            sup = Some(Box::new(self.parse_single_or_group()));
        }
        ...
        match (sub, sup) {
            ...
            (None, None) => self.try_parse_scripts(base),
        }
    }
...
#[cfg(test)]
mod tests {
    use super::super::symbols::{DecoKind, FontStyleKind};
    use super::*;
...
mod latex_compat_tests {
    ...
    #[test]
    fn official_deg_case_distinguishes_function_and_symbol() {
        assert_eq!(
            parse("deg DEG"),
            EqNode::Row(vec![
                EqNode::Function("deg".to_string()),
                EqNode::MathSymbol("°".to_string()),
            ])
        );
    }
