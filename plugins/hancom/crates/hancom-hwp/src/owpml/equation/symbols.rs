...
//! RHWP `src/renderer/equation/symbols.rs`를 커밋
//! `496333b27d21ddb9114ba9ae340bcb895870c9a7`에서 가져와 공식 r1.3 함수와
//! 엄격 변환 경계를 보완했다. 라이선스는 `plugins/hancom/NOTICE`에 있다.
...
static OPERATORS: LazyLock<HashMap<&'static str, &'static str>> = LazyLock::new(|| {
    HashMap::from([
        ...
        ("LOR", "∨"),
        ("XOR", "⊻"),
        // 공식 표에서 이들은 첨자 없는 이항 기호다. BIG* 변형과 구분한다.
        ("SQCUP", "⊔"),
        ("SQCAP", "⊓"),
        ("UPLUS", "⊎"),
        ("OPLUS", "⊕"),
...
pub fn is_function(cmd: &str) -> bool {
    FUNCTIONS.contains_key(cmd) || FUNCTIONS.contains_key(cmd.to_ascii_lowercase().as_str())
}
...
pub fn lookup_symbol(cmd: &str) -> Option<&'static str> {
    ...
    // r1.3은 소문자 `deg`를 로만체 함수, 대문자 `DEG`를 도 기호로 정의한다.
    if upper != cmd && !cmd.eq_ignore_ascii_case("deg") {
...
pub fn lookup_function(cmd: &str) -> Option<&'static str> {
    FUNCTIONS
        .get(cmd)
        .or_else(|| FUNCTIONS.get(cmd.to_ascii_lowercase().as_str()))
        .copied()
}
