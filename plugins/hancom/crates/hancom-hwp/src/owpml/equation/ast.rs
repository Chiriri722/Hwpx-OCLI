...
//! RHWP `src/renderer/equation/ast.rs`를 커밋
//! `496333b27d21ddb9114ba9ae340bcb895870c9a7`에서 가져와 손실 없는 OfficeCLI
//! 변환에 필요한 노드를 보완했다. 라이선스는 `plugins/hancom/NOTICE`에 있다.
...
pub enum EqNode {
    ...
    /// 왼쪽 아래첨자: x LSUB a → {}_a x
    LeftSubscript { base: Box<EqNode>, sub: Box<EqNode> },

    /// 왼쪽 위첨자: x LSUP b → {}^b x
    LeftSuperscript { base: Box<EqNode>, sup: Box<EqNode> },

    /// 왼쪽 위·아래첨자: x LSUB a LSUP b → {}_a^b x
    LeftSubSup {
        base: Box<EqNode>,
        sub: Box<EqNode>,
        sup: Box<EqNode>,
