...
fn detect_cfb<R: Read + Seek>(reader: R) -> Result<SourceFormat> {
    let mut comp = cfb::CompoundFile::open(reader).map_err(|e| {
        PluginError::corrupt(format!("not a readable CFB container: {e}"))
    })?;
...
fn detect_zip<R: Read + Seek>(reader: R) -> Result<SourceFormat> {
    ...
    let has_section = archive.file_names().any(|n| {
        n.starts_with(crate::owpml::package::SECTION_PREFIX) && n.ends_with(".xml")
    });
    if has_section {
        return Ok(SourceFormat::Hwpx);
    }
...
mod tests {
    ...
    fn hwpx_bytes(mimetype: Option<&str>, with_section: bool) -> Vec<u8> {
        let mut cursor = Cursor::new(Vec::new());
        {
            ...
            if with_section {
                zip.start_file("Contents/section0.xml", opts).expect("start");
    ...
    fn rejects_zip_with_wrong_mimetype() {
        ...
        let e = detect_reader(Cursor::new(b)).expect_err("must reject");
        assert!(e.message.contains("unexpected mimetype"), "got: {}", e.message);
    }
    ...
    fn rejects_cfb_without_file_header() {
        ...
        assert!(e.message.contains("not an HWP document"), "got: {}", e.message);
    }
    ...
    fn rejects_unrecognized_bytes_with_a_hex_hint() {
        let e = detect_reader(Cursor::new(b"this is plain text".to_vec()))
            .expect_err("must reject");
        assert!(e.message.contains("unrecognized format"), "got: {}", e.message);
