...
> **진행 상태**: H2(포맷 판별) 완료. 다음은 H3(rhwp 브리지).
> 3절 각 항목 아래에 완료 기록을 덧붙였다.
...
| 언어 | **Rust** | TypeScript |
| stars | 3,623 | 1,583 |
| 최근 push | 2026-07-31 (당일) | 2026-07-21 |
...
- 바이너리가 507KB → 수 MB로 커진다.
- rhwp가 0.8.2이고 매우 빠르게 움직인다(당일 push). rev 고정 + 주기적 갱신 부담.
...
1. **CFB 시그니처만으로는 부족하다.** `.doc`/`.xls`가 같은 컨테이너다.
   ...
   추가했다(MIT, 4,900만 다운로드, rhwp도 같은 걸 쓴다). 바이너리 507K → 555K.
...
바이너리 HWP는 지금 **exit 3**(`unsupported_feature`)으로 나가며 무엇을 해야
하는지 알린다. 조용히 실패하거나 "zip을 못 읽는다"로 오인시키지 않는다:

```
$ officecli-dump-reader-hwpx dump 문서.hwp
[unsupported_feature] this is a binary HWP 5.x document (version 5.1.0.1), not
HWPX. This build reads HWPX only. Convert it first, e.g.:
...
**H3에서 바뀌는 것**: `needs_conversion()`이 참일 때 exit 3 대신 변환기를 호출한다.
판별 결과(`SourceFormat`)를 그대로 넘기면 되므로 인터페이스는 준비돼 있다.
...
fn convert_hwp_to_hwpx(src: &Path, scratch: &Path) -> Result<PathBuf>
```
...
### H4. HWP↔HWPX 동등성 회귀 (반나절)

같은 문서의 두 포맷을 갖고 있으므로 **출력 대조**가 가능하다.
`scripts/verify-corpus.py`를 확장한다.

- `X.hwp`와 `X.hwpx`가 모두 있으면 두 JSONL의 요약을 비교한다.
- 비교 대상: 항목 수, 문단·표·셀·폼필드 개수, 텍스트 문자 다중집합.
- **완전 일치를 요구하지 않는다.** RHWP 변환은 무손실이 아니고, 사용자가 한쪽만
  편집했을 수도 있다(실측: `참가신청서 P1.hwp`와 `P1.hwpx`의 내용이 달랐다).
  허용 오차를 기준선에 박아둔다.

### H5. 문서화 (반나절)

- README에 HWP 지원과 rhwp 설치 방법.
- `docs/01-protocol-contract.md`에 ADR-5(A1 선택), ADR-2 정정.
- `docs/02-handover.md`에 "설명문만 보고 판단해서 틀린 다섯 번째 사례" 기록.

### 순서

```
H2 판별  →  H3 브리지  →  H1 디스커버리  →  H4 회귀  →  H5 문서
```
...
| HWP↔HWPX 동등성 | `verify-corpus.py`의 쌍 비교 |
...
| R4 | 암호 걸린 HWP | rhwp에 `crypto.rs`가 있으니 변환기가 처리할 수 있다. 우리 쪽 표본이 없어 미검증 |
| R5 | HWP 3.0 | rhwp `hwp3/`가 있다. 확장자가 같은 `.hwp`이므로 H2 판별로 자동 처리된다. 표본 없음 |
| R6 | 성능. 변환 단계가 늘어난다 | dump는 일회성이고 호스트가 형제 `.docx`를 캐시한다(§2.1). 큰 파일 실측 필요 |
...
H5에서 ADR-2를 정정하고 ADR-5(A1 채택)를 추가한다.
